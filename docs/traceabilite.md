...
**Mis à jour le :** 2 septembre 2026, en fin de chantier — six extracteurs,
**266 tests**, **41 décisions**, **16 dossiers d'incident**, **21 réserves**. Les écarts du premier relevé sont repris en fin de
...
| C3 | Agrégation et nettoyage des données | **vérifié** | Trois modules distincts — normalisation des dates (ISO 8601), homogénéisation des formats, déduplication. 7 880 entrants, 40 doublons d'identifiant retirés, 0 doublon de contenu, 7 840 sortants, 0 date perdue. Clés de déduplication sur identifiant et empreinte SHA-256 du contenu, jamais sur l'URL ni le titre | `data_pipeline/transform/` (6 modules) |
| C4 | Base de données et modélisation | **vérifié** | Base `eduai_data` en PostgreSQL 16 conteneurisé, **13 tables et 4 vues**, **7 840 documents** chargés — S6 comprise depuis le 2 septembre, 1 211 mots-clés, 20 544 rattachements, 0 rejet. **S4 est exercée et rend 0 document** : ses deux requêtes cherchent des paires « échec puis correction » et des erreurs conceptuelles, et les deux sources étaient vides le 27 août — le corpus des apprenants se remplit à l'usage, non à la livraison. MCD, MLD, dictionnaire de données, document de conformité RGPD. Isolation structurelle d'avec `eduai_app` (décision 006) | `data_pipeline/load/`, `docs/mcd_eduai_data.md`, `docs/mld_eduai_data.md`, `docs/dictionnaire_donnees_eduai_data.md`, `docs/rgpd_eduai_data.md`, décisions 006, 007, 009. **L'attribution voyage jusque dans la fiche de l'apprenant** : url, licence et obligation d'attribution sont conservées par ajout — c'est la condition juridique qui autorise l'usage de ce corpus (décision 041) |
| C5 | API REST exposant le jeu de données | **vérifié** | Django REST Framework, 3 jeux de vues et une vue de statistiques, lecture seule garantie par un routeur de base qui lève sur écriture. **7 758 documents exposés sur 7 840** — les 82 documents de licence non vérifiée et les productions d'apprenants sont retenus par `redistribution_autorisee` — le filtre de licence est dans le queryset de base, pas dans chaque vue. Authentification, permissions, throttling, pagination, OpenAPI | `apps/api_data/`, `docs/securite_api_donnees.md`, décisions 012, 013 |
